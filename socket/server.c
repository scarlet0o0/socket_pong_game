...
int main(void){
	...
	char *cname[]={"Chateoom-0","Chatroom-1","Chatroom-2"};//채팅방 이름
			...
			while(ret >0){
				if(FD_ISSET(comm_sock[i],&readfds)){
					...
					if(recv(comm_sock[i],buf,sizeof(buf),0) >0){
						printf("[MAIN]사용자%d 의 메시지 :%s",comm_sock[i], buf);
						menu_num = buf[0]-'0';
						switch(menu_num){
			...
			while(ret>0){
				if(FD_ISSET(chatinfo->room.user[l],&readfds)){
					...
					if(recv(chatinfo->room.user[l],buf,BUFLEN,0) > 0) {
						printf("[ch.%d] 사용자 %d 의 메세지 : %s",chatinfo->room.room_id,chatinfo->room.user[l],buf); 
